...
class ProductsController extends Controller
{
  ...
  public function updateAttributeStatus(Request $request){
    ...
    //    return response()->json(['status' => $status, 'attribute_id' => $data['attribute_id']]);
       $response = ['status' => $status, 'attribute_id' => $data['attribute_id']];
       return response()->json($response, 200);
    }
   }
   public function editAttributes(Request $request){
    ...
    if ($request->isMethod('post')) {
      ...
      if (isset($data['attributeId']) && is_array($data['attributeId'])) {
          ...
          return redirect()->back()->with('error_message', 'No attributes found to update.');
      }
      }
      //  if($request->isMethod('post')){
      //      $data = $request->all();
  ...
  public function addImages($id, Request $request){
    Session::put('page','products');
    $product = Product::select('id','product_name','product_code', 'product_color','product_price'
    ,'product_image')->with('images')->find($id);
